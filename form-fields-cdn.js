...
try {
        ...
        if (
          ...
          this.conf.range
            ? ((this.values.start = this.conf.values.indexOf(t[0])),
              (this.values.end = this.conf.set[1] ? this.conf.values.indexOf(t[1]) : null))
      ...
      (t.prototype.setValues = function (t, i) {
        ...
        return (
          ...
          this.conf.range
            ...
            : (this.conf.tooltip && (this.tipL.innerHTML = this.conf.values[this.values.end]),
  ...
  console.log("Error attaching range slider library script to DOM: ", err);
}
